...
transcrips <- as.numeric(c("526","723","1064","821","697","743","1149","489"))

# What is the length of the smallest transcript?
min(as.numeric(transcrips))
## [1] 489
...
tr <- cbind(tr, tr)
colnames(tr) <- c("speciesA", "systerA")
...
rownames(tr) <- c("BAL", "CAM", "LSD", "FUZ", "ZERP", "DING", "NOP", "YIN")

# Sort the genes alphabetically and save the sorted gene list in a new vector.

tr2 <- tr[order(rownames(tr), decreasing = F), ] ; 
...
list1 <- c("NOP", "PHO", "SEN")
list2 <- c("dns", "hro", "lamp", "bal", "cat", "krr", "zerp", "yin", "tir", "jog", "nop")
...
Table1=read.table("/media/public/friday/Matrix1.txt", header=TRUE, sep="\t", quote="", stringsAsFactor=FALSE) 
...
Table1=read.table("/media/public/friday/Matrix1.txt", header=TRUE, sep="\t", quote="", row.names=1, 
...
Table2=read.table("Matrix2.txt", header=TRUE, sep="\t", 
                    stringsAsFactor=FALSE)
...
colnames(Table2)
# [1] "gene"   "brain1" "brain2" "brain3" "heart1" "heart2" "heart3" "lung1"  "lung2"  "lung3" 
# [11] "liver1" "liver2" "liver3"
# Which genes (column 1) have expression values larger than 30 (column 3)
Table2[which(Table2[, 3] > 30), 1]
# [1] "Hspa5" "Hspa4" "Cpsf2" "Otx2"  "Cpsf3" "Hspa8" "Med28"
# Is the gene Med31 in the table?
any(Table2[, 1] == "Med31")
# TRUE
# How often is Med31 in the table?
length(which(Table2[1] == "Med31"))
# [1] 4

# Which Expression values belong to Med31?
Table2[which(Table2[1] == "Med31"),]
Table2[Table2[1] == "Med31",]
...
Transcripts <- c("526",  "723", "1064",  "821",  "697",  "743", "1149", "489",  "762")
Transcripts <- as.numeric(Transcripts)  
